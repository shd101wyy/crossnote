...
export default class WebviewController {
  ...
  private vscodeApi: {
    ...
    postMessage: (args: { command: string; args: any[] }) => void;
  } | null = null;
   ...
   * .sidebar-toc element
   */
  private sidebarTOC: HTMLElement | null = null;
  ...
  private refreshingIconTimeout: any = null;
   ...
   * Markdown file URI
   */
  private sourceUri: string | null | undefined = null;
   ...
   */
  constructor({ previewElement }: { previewElement: HTMLDivElement }) {
  ...
  private postMessage(command: string, args: any[] = []) {
    if (this.config.vscode) {
      if (!this.vscodeApi) {
        ...
        // @ts-ignore
        this.vscodeApi = globalThis.acquireVsCodeApi();
      }
    ...
    private initImageHelper() {
      ...
      urlEditor.addEventListener('keypress', (event: KeyboardEvent) => {
        if (event.keyCode === 13) {
          ...
          if (url.length) {
            window['$']['modal'].close(); // close modal
      ...
      copyLabel.innerText = `Copy image to ${
        this.config.imageFolderPath && this.config.imageFolderPath[0] === '/'
          ? 'workspace'
          : 'relative'
      ...
      dropArea.on('click', function(e: Event) {
        e.preventDefault();
        e.stopPropagation();
        window['$'](this)
  ...
  private async renderWavedrom() {
    const els = this.previewElement.getElementsByClassName('wavedrom');
    if (els.length) {
      const wavedromCache = {};
  ...
  private renderMathJax() {
    return new Promise<void>(resolve => {
      if (
        ...
        const unprocessedElements = this.previewElement.querySelectorAll(
          '.mathjax-exps, .math.inline, .math.display',
        );
        ...
        window['MathJax'].typesetPromise([this.previewElement]).then(() => {
          ...
          if (!this.previewElement.getElementsByClassName('MathJax').length) {
            return resolve();
          }
   ...
   * @param id
   */
  private runCodeChunk(id: string | null) {
    if (!this.config.enableScriptExecution || !id) {
      return;
    }
  ...
  private renderSidebarTOC() {
    if (!this.enableSidebarTOC || !this.sidebarTOC) {
      return;
    }
   ...
   */
  private buildScrollMap(): number[] | null {
  ...
  // TODO: Refactor this
  /*
    private escPressed(event: Event | null = null) {
      if (event) {
        event.preventDefault();
        event.stopPropagation();
      }
