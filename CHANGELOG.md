...
- **The mermaid and wavedrom CLI wrappers no longer run through a shell (CVE-2022-45026)** — `tools/mermaid.ts` spawned `@mermaid-js/mermaid-cli` with `shell: true`, so the PNG output path it was handed reached cmd.exe/sh as part of a command line rather than as an argument. That path is built from the diagram's `filename` attribute — untrusted markdown — plus the notebook's `imageFolderPath` and project directory, and only the basename passes `sanitizeImageFilename()`; the directory part was never checked, so a project or image folder whose name contains a shell metacharacter was enough to inject a command on export. `tools/wavedrom.ts` had the identical pattern. Both now spawn through `cross-spawn` with no shell of our own, matching the shell-free spawning of `tools/pdf.ts` and `tools/latex.ts`, which were hardened in [0.9.29](#0929---2026-06-05). Windows still resolves the launcher correctly: `cross-spawn` routes `npx` through `cmd.exe` itself with cmd metacharacters escaped — plain `spawnSync('npx.cmd', …)` cannot work there at all, since Node ≥ 18.20.2 (CVE-2024-27980 hardening) refuses to launch `.cmd`/`.bat` files without a shell and throws `EINVAL` ([#512](https://github.com/shd101wyy/crossnote/pull/512) by @Waynting; [#274](https://github.com/shd101wyy/crossnote/issues/274) reported by @yuriisanin).
...
- **Local fonts and images in `.crossnote/style.less` load again** — a relative `url(...)` in the user stylesheet (`@font-face { src: url('fonts/MyFont.woff2') }`, `background-image: url(./bg.png)`) never resolved: the compiled CSS is inlined into a `<style>` tag, so the browser resolved those references against the _preview document_ rather than against style.less — inside a VS Code webview that is `vscode-webview://<uuid>/`, where nothing exists. Local fonts silently fell back to the default while remote ones (Google Fonts, any `https://` URL) kept working, which is what made the failure look arbitrary ([vscode-mpe#2424](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2424) reported by @patriknusszer). Relative references are now resolved when style.less is loaded — the last point at which its own directory is known, since a host may concatenate the global and workspace stylesheets into one `globalCss` afterwards — and turned into loadable URLs at render time (`vscode-webview://…` in the preview, `file://…` for exports and `crossnote serve`) ([#513](https://github.com/shd101wyy/crossnote/pull/513) by @Waynting). Absolute URLs, `url(#fragment)` paint-server references, paths that escape the stylesheet's directory, and references to files that do not exist are all left exactly as they were. Root-relative `url(/x)` is left alone at load time but — being textually identical to an already-resolved POSIX path — is treated as a filesystem-absolute path at render time. On Windows the load-time resolution also survives drive-relative notebook roots (`crossnote serve`/`build-wiki` round-trip plain `C:\…` paths through `URI.parse`, which drops the drive letter): the stylesheet directory is normalized through `path.resolve` before the containment comparison, and the CSS string escapes are undone before the path is mapped, so hosts receive real filesystem paths.
- **`crossnote serve` keeps root-relative `url(...)` in style.less working** — a follow-up to the entry above: treating root-relative references (`url(/files/bg.png)`) as filesystem-absolute paths at render time sent them through serve's URL mapper, which recognized none of its own routes and fell back to `file:///files/bg.png` — a URL a browser refuses to load from an `http://127.0.0.1` page, so a reference that had loaded before the rewriting landed silently broke. Serve's mapper now recognizes its own route prefixes (`/files/`, `/assets/`) and passes them through unchanged, letting the page resolve them against the origin; genuinely absolute paths outside every served root keep the `file://` fallback ([#516](https://github.com/shd101wyy/crossnote/pull/516) by @shd101wyy).

### Improvements

- **Vendored mermaid upgraded from 11.17.2 to 12.0.0** — the preview and the offline/CDN HTML export now render diagrams with mermaid 12 (the CDN fallback URL and `package.json` follow along; the bundle stays mermaid's official jsDelivr dist build, now ~5.5 MB). mermaid 12 declares `engines.node >= 22.12.0` and its sources target ES2024, which would clash with crossnote's Node 18 runtime floor — but crossnote never executes the mermaid npm package in Node (the `MermaidConfig` import is type-only), and the shipped IIFE bundle still parses on Node 18 and contains no ES2024 runtime APIs newer than the VS Code webview floor of Electron 25 / Chromium 114 (no `Promise.withResolvers`, `Object.groupBy`/`Map.groupBy`, or `Array.fromAsync`). Two guard tests lock those properties in — the bundle must keep parsing under the CI Node 18 and must not reference Chromium 116+ APIs — so a future mermaid bump cannot silently break older webviews.
...
- **`crossnote build-wiki` CLI — single-file read-only wiki (a la TiddlyWiki)** — `npx crossnote build-wiki [directory…] [-o index.html]` packs the directories' markdown notes into one standalone HTML file: every note is rendered through the regular HTML export pipeline (CDN-hosted KaTeX/MathJax/mermaid/wavedrom/vega assets, local images and SVGs embedded as data URIs so the file travels on its own) and the documents are embedded into a shell with a small client-side app — a fuzzy-filtered file list and a `sandbox="allow-scripts"` iframe showing one pre-rendered note at a time, with note-to-note links resolving inside the wiki, external links opening new tabs, and task-list checkboxes disabled. Read-only by construction: nothing in the file can write anywhere. Config resolution matches `serve` (`defaults ← VS Code settings with --vscode ← global crossnote config ← <directory>/.crossnote`, per directory); notes that fail to render are reported and skipped, and the build prints the output size. The engine method behind it is factored out of `htmlExport` as `MarkdownEngine.htmlExportDocument()` (renders the exact export document without touching the filesystem; `htmlExport` now calls it), the per-directory Notebook bootstrap is shared with `serve` (`createNotebooksForDirectories`), and `buildWiki` is exported from the library for host applications. The preview context menu gains an **Export standalone HTML (wiki)** item — hidden in the web extension — which builds the same file: `crossnote serve` writes it next to the notes as `crossnote-wiki.html` (timestamped if that exists, so re-exports never clobber) and reports the path via a new `notification` SSE event that the server app shows as a toast.
- **`crossnote serve` CLI — standalone markdown preview server with a browser app** — `npx crossnote serve [directory…]` starts an HTTP server (default `http://127.0.0.1:3000`) that renders the directories' markdown — multiple directories are supported like a VS Code multi-root workspace (each folder keeps its own `.crossnote` config; `/files` URLs carry a root hint so same-named files never collide) with the same preview as the VS Code extension. The browser app (`/`) provides a VS Code–like layout: a `Ctrl/Cmd+P` fuzzy file picker, tabs, resizable split panes, tab drag & drop (drop on a pane edge to split there), zen mode, and a welcome page. Previews run the unmodified webview bundle in same-origin iframes behind an injected `acquireVsCodeApi` shim; the webview↔host protocol is relayed over HTTP/SSE, so disk changes re-render previews live, the in-preview editor writes back to the file, task-checkbox clicks toggle the source, and code chunks run when `enableScriptExecution` is on. Config follows the extension's merge order (`defaults ← VS Code settings with --vscode ← global crossnote config ← workspace .crossnote`); theme changes persist to VS Code `settings.json` (comment-preserving) or the global `config.js`. Host-only context-menu items (open-in-browser, exports, image helper, …) are hidden via the new `WebviewConfig.isServerApp` flag. The HTTP server is hardened for its loopback context: requests are gated on `Host`/`Origin` naming the server itself (websites open in the same browser cannot drive `/api/command` — which runs code chunks and writes files — and DNS rebinding cannot read responses), and HTML/SVG/XML files served from the directories are delivered with a `CSP: sandbox` header so a document from an untrusted repository never executes same-origin script against the app. The CLI is also embeddable by host applications: it is importable as `crossnote/cli`, `--json` emits a machine-readable startup line, and `--build-dir` points it at a host-provided build directory — the surface used by vscode-markdown-preview-enhanced's server command.
...
- **Reuse the sidebar TOC markdown-it instance** — `parseMD` built a fresh markdown-it instance with the full plugin chain (18 plugins) on every render just to generate the sidebar TOC HTML. The instance is now created lazily once per notebook and reused (`this.md.options` never change after construction).
...
- **Browser/HTML exports render in the configured theme again; following the reader's OS is now opt-in (`exportColorScheme`)** — [0.9.32](#0932---2026-08-29) made every screen export (Open in Browser, HTML export) follow the reader's system color scheme: the configured preview/code-block theme only selected which light/dark pair to embed, and the reader's OS picked the variant — so a document explicitly themed light rendered dark on a dark-OS machine, and one explicitly themed dark rendered **light** on a light-OS machine. Screen exports are deterministic again: by default they render exactly in the configured themes on every machine (this also keeps the 0.9.32 fix of no longer forcing `github-light` when `printBackground` is off). Following the reader's system color scheme — light/dark pairs under `@media (prefers-color-scheme: dark)` plus `<meta name="color-scheme" content="light dark">` — is now opt-in via the new `exportColorScheme: 'auto'` notebook config, or per document with front matter `html: { export_color_scheme: auto }`. Paper output (Chrome PDF / Prince) keeps its forced-light behavior ([vscode-mpe#2382](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2382) reported by @dada-xiv).
- **Fix preview crash on callouts whose marker line is its own paragraph** — the callout renderer stripped the `[!NOTE]` marker line by nulling the paragraph's inline-token `children` and hiding the paragraph tokens, but markdown-it's `Renderer.render()` dispatches inline tokens without honoring `hidden` (only `renderToken` checks it, for block tokens), so `renderInline(null)` crashed with `TypeError: Cannot read properties of null (reading 'length')`. Any callout written with the marker line standing alone or followed by a blank quoted line — `> [!NOTE]`, blank `>`, then the body — killed the entire preview. Regression from [#463](https://github.com/shd101wyy/crossnote/pull/463) in [0.9.32](#0932---2026-08-29); the children are now emptied to an array instead ([vscode-mpe#2375](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2375) reported by @GodExious).
- **Fix footnotes rendered inside a callout title** — the callout title was rendered with `md.renderInline(title, env)`, and `renderInline`'s parse re-runs core rules with the document env; markdown-it-footnote's `footnote_tail` then re-emitted an empty copy of the document's footnotes section inside the callout title, in addition to the real one at the bottom. The title is now rendered with a fresh env, so footnotes appear exactly once, at the document bottom ([vscode-mpe#2377](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2377) reported by @chz-cn).
- **Fix `{highlight=...}` band rendering below the code block in most code themes** — only the `vue` and `vscode` code themes carried the structural CSS (`position: relative` on `pre[data-line]`, absolutely positioned wrapper/band); `github-dark` had it partially (no wrapper positioning) and every other prism theme not at all, so the appended highlight divs rendered in flow _below_ the code. The structural rules now live in the shared `style-template.css` (included by the preview and every export path) as the single source of structure, with a translucent default band; themes override the band color via the `--line-highlight-background` custom property (`vscode` blends VS Code's selection color, `github-dark` keeps its tuned tone), so the previously duplicated structural rules were removed from those themes ([vscode-mpe#2379](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2379) reported by @chz-cn).
- **Fix `{highlight=...}` band covering code text with the `vscode` code theme** — the band used `--prism-selection` (VS Code's `editor.selectionBackground`, opaque in most themes) as its background, painted over the text. It is now blended to 35% opacity via `color-mix` (the opaque color remains as a fallback for engines without `color-mix` support), and the variable's fallback default was normalized from an out-of-range `rgba(264,264,264,…)`, which only worked through clamping, to `rgba(128,128,128,0.25)` ([vscode-mpe#2378](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2378) reported by @chz-cn).
...
- Fix note titles keeping the file extension on non-`.md` notebooks — `path.basename(absFilePath).replace(/\.md$/, '')` only stripped literal `.md`, so a notebook configured with `.markdown` / `.mdx` / `.qmd` produced titles like `readme.markdown`. Now strips whichever configured extension matches.
...
- **Lazy markdown bodies** — `Note.markdown` is no longer part of the in-memory cache (the field is removed from the `Note` type entirely). `Notebook.getNoteMarkdown(filePath)` is the canonical way to fetch a body — reads from disk on demand, applies the same `markdownFileExtensions` / `maxNoteFileSize` gates as `getNote`. For prose-heavy notebooks this drops cache RSS by roughly the total markdown size — a 1 000-note notebook averaging a few KB per note saves several MB. As a side benefit, on-disk edits are picked up by `getNoteMarkdown` without needing a `refreshNotes` round-trip first. **Breaking change** for any external code reading `note.markdown` — switch to `await notebook.getNoteMarkdown(filePath)`.
- **Token-free `Reference`** — the public `Reference` shape no longer carries markdown-it `Token` objects; instead each reference holds a pre-rendered `html: string` (the surrounding inline context the Backlinks panel renders) plus an optional `sourceLine?: number` (used for `#L<n>` click-through). HTML is computed once at index time inside `processNoteMentionsAndMentionedBy` so the cache holds strings instead of nested Token trees. **Breaking change** for any external code reading `reference.parentToken` / `reference.token` — switch to `reference.html` and `reference.sourceLine`.
- **Incremental refresh** — new `Notebook.refreshNotesIncremental(args)` API. Walks the workspace, diffs each file's on-disk mtime against an internal `processedMtimes` map, and only re-processes files whose mtime advanced (plus newly-added files); files that disappeared from disk get their cache + reference / tag entries removed. Designed for file-system watcher callbacks where one file changed and you don't want to re-tokenise the other 999. The full-rebuild `refreshNotes` is unchanged and still the right call for first-load. Both share the `refreshNotesMutex` so they can't race each other.
...
The eager in-memory note cache is now slim: it holds metadata (title, aliases, front-matter-derived `NoteConfig`), the search index, and the reference / tag graph. Per-reference, only the rendered HTML and an integer source line are retained — no markdown-it `Token` trees, no raw markdown bodies. Both the body strings and the token trees are lazy-loaded / re-rendered on demand: `Notebook.getNoteMarkdown(filePath)` re-reads the body from disk; the Backlinks panel reads `reference.html` directly, no re-render at panel time. The `maxNoteFileSize` cap continues to mitigate the worst pathological case (a single huge committed file). Remaining future-release consideration: an mtime-keyed parse cache that survives Notebook restarts (currently `processedMtimes` is process-local).

### Internal

- Added 44 new tests across 5 test suites for wikilink embedding, block references, tag parsing, lazy markdown loading, reference HTML pre-rendering, incremental refresh, explicit `{#id}` heading-fragment resolution, ATX-cap regex consistency, and per-extension title stripping
