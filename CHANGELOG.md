...
- Fix `findFragmentTargetLine` heading regex matching invalid 7+ hash lines — `extractHeadings` already enforced the markdown ATX cap (`^#{1,6}\s+`), but `findFragmentTargetLine` accepted any number of leading hashes. Now both passes use the same `^#{1,6}\s+` shape, so a paragraph starting with seven hashes can't shadow a real heading further down the file.
...
- Defensive guard around `processedMtimes.set(...)` in `refreshNotesIncremental` — replaced `onDisk.get(filePath)!` with a runtime null-check, so any future drift between path normalisation in `loadNoteFromDisk` vs `_collectOnDiskMtimes` (e.g. Windows separator handling) skips the stamp instead of writing `undefined`.
...
- **Lazy markdown bodies — `Note.markdown` removed entirely** — the note body now lives on disk only. `Note.markdown` (previously `string`, then briefly `markdown?: string` while we landed eviction) is gone from the type. `Notebook.getNoteMarkdown(filePath)` is the only way to fetch a body — always reads from disk, applies the same `markdownFileExtensions` / `maxNoteFileSize` gates as `getNote`. Internally, `loadNoteFromDisk` returns `{ note, markdown }`, the body lives in a local for as long as the caller needs it, then falls out of scope; `processNoteMentionsAndMentionedBy(filePath, markdownOverride?)` accepts pre-loaded markdown so refresh paths thread it through without re-reading. `_refreshNotesInternal` collapses walk-and-process into a single pass — the body never enters `this.notes`, so the post-walk eviction loops in `refreshNotes` and `refreshNotesIncremental` are gone. For prose-heavy notebooks this drops cache RSS by roughly the total markdown size; the type-level removal also makes the contract impossible to misuse (TypeScript flags any leftover `note.markdown` reads as compile errors). As a side benefit, on-disk edits are picked up by `getNoteMarkdown` without needing a `refreshNotes` round-trip first. **Breaking change** for any external code reading `note.markdown` — switch to `await notebook.getNoteMarkdown(filePath)`.
- **Token stripping in `Reference`** — `Reference.parentToken` and `Reference.token` (markdown-it `Token` objects) are removed from the public shape. They've been replaced by a pre-rendered `html: string` (the surrounding inline context, what the Backlinks panel would have rendered), plus an optional `sourceLine?: number` (the parent token's `.map[0]`, used by the Backlinks panel for `#L<n>` click-through). HTML is computed once at index time inside `processNoteMentionsAndMentionedBy` so the cache no longer pins markdown-it Token trees. For a notebook with many wikilinks this is the second-largest contributor to cache footprint after raw markdown bodies (handled above). Strings compress better than nested Token objects with content slices, so the net memory footprint per reference shrinks. **Breaking change** for any external code reading `reference.parentToken` / `reference.token` — switch to `reference.html` (renders identically) and `reference.sourceLine`.
- **Incremental refresh via mtime cache** — new `Notebook.refreshNotesIncremental(args)` API. Instead of `refreshNotes`'s wipe-and-rebuild, it walks the workspace, diffs each file's on-disk mtime against an internal `processedMtimes` map, and only re-processes files whose mtime advanced (plus newly-added files); files that disappeared from disk get their cache + reference / tag entries removed. Designed for file-system watcher callbacks where one file changed and you don't want to re-tokenise the other 999. The full-rebuild `refreshNotes` is unchanged and still the right call for first-load. Both share the `refreshNotesMutex` so they can't race each other.
...
- DRY refactor of the gitignore-aware directory walk — `_refreshNotesInternal` (full rebuild) and `_collectOnDiskMtimes` (incremental refresh) used to each carry their own copy of the gitignore stack construction, `isIgnoredByGitignore` closure, `node_modules` / `.git` skip, and recursion semantics. Both now drive their work through a shared `_walkNotebookDir(dir, includeSubdirectories, gitignoreStack, onFile)` helper, so the gitignore semantics live in exactly one place. Side win: `_refreshNotesInternal` used to stat each entry twice (once via `getNote → loadNoteFromDisk`, once explicitly to decide recursion); the helper stats once and routes off that single result.
